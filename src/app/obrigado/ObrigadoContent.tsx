...
/**
 * Iscas que já dispararam ViewContent NESTE carregamento de página.
 *
 * Mora no módulo, e não num `useRef`, de propósito: o componente vive dentro do
 * <Suspense> que lê os searchParams e é REMONTADO quando eles resolvem — um ref
 * novo nasce junto com a instância nova e não segura nada. Sem esta guarda, parte
 * dos carregamentos disparava ViewContent duas vezes no mesmo segundo e dobrava a
 * métrica de fundo de funil da isca (medido em 2 de 4 aberturas, 01/Set/2026).
 */
const viewContentDisparado = new Set<string>();
...
function ObrigadoInterno() {
  ...
  // Qual guia esta página está entregando. `resolverIscaId` aceita o id canônico e os
  // apelidos (o nome comercial do guia escrito como id); sem `?isca=`, ou com um id de
  // fato desconhecido, cai no padrão — links antigos seguem mostrando o Cadeira Cheia.
  // MESMA função usada pelo generateMetadata do page.tsx: aba e conteúdo não divergem.
  const iscaId = resolverIscaId(searchParams.get("isca"));
  ...
  //
  // O título da aba NÃO é mais ajustado aqui: o `document.title` imperativo perdia para
  // o <title> que o Next monta a partir do metadata (o valor final na aba era o genérico
  // "Seu guia está aqui"). Quem nomeia o guia na aba agora é o `generateMetadata` do
  // page.tsx, que roda no servidor e por isso acerta em qualquer momento do carregamento.
  useEffect(() => {
