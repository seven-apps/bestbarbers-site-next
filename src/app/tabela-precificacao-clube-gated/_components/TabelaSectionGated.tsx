...
import { SliderInput } from "../../tabela-precificacao-clube/_components/SliderInput";
import { AnimatedMoney } from "../../tabela-precificacao-clube/_components/AnimatedMoney";
import {
  AGENDA_OPCOES,
  DEFAULTS,
  LIMITES,
  ...
  calcular,
  type CalcInputs,
} from "../../tabela-precificacao-clube/_components/calc";
import { METODO } from "../../tabela-precificacao-clube/_components/benchmarks";
...
export function TabelaSectionGated({ unlocked = false, onUnlockRequest }: Props) {
    ...
    >
      ...
      <div className="container-custom relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 max-w-6xl mx-auto items-stretch">
          {/* ── COLUNA 1 — Inputs (LIVRE, sempre interativa) ── */}
          <div
          ...
          {/* ── COLUNA 2 — Resultado ao vivo (GATED: borrado até o form) ── */}
          <div
          ...
          >
            <div
              className="px-6 md:px-8 py-8 md:py-10 flex flex-col gap-5 h-full"
              // Enquanto bloqueado: borra e torna o resultado ilegível +
              // não-selecionável + não-clicável. Nada é afirmado ao usuário.
              style={
                !unlocked
            ...
            {/* Overlay do gate — só aparece enquanto bloqueado */}
            {!unlocked && (
              <div
                className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-6"
                style={{
                  background:
                    "linear-gradient(160deg, rgba(10,10,10,0.72) 0%, rgba(10,10,10,0.82) 100%)",
              ...
              >
                <div
                  ...
                  style={{
                    background: "rgba(235,173,4,0.14)",
                    border: "1px solid rgba(235,173,4,0.4)",
                  }}
                >
                  <Lock className="w-6 h-6" style={{ color: "#ebad04" }} />
                </div>
                <p
                  className="text-[15px] md:text-[16px] font-bold leading-snug mb-5 max-w-xs"
                  style={{ color: "#ffffff", fontFamily: "var(--font-montserrat)" }}
                >
                  Preencha o formulário para revelar o preço recomendado da sua assinatura
                </p>
                <button
                  type="button"
                  onClick={onUnlockRequest}
                  className="text-white font-extrabold text-[15px] md:text-[16px] px-6 py-4 rounded-full transition-all duration-300 flex items-center justify-center gap-2 active:scale-[0.98]"
                  style={{
                    background: "linear-gradient(135deg, #029912, #02ab15)",
                    boxShadow: "0 4px 14px 0 rgba(2,171,21,0.39)",
                    fontFamily: "var(--font-montserrat)",
