...
interface TotemClubeProps {
  onCtaClick: () => void;
}

/**
 * Clone da TotemSection da homepage para /clube.
 * Headline "Adquira também: o Totem que recebe o seu assinante".
 * originDesc: [Site-Clube]BT-Totem.
 */
export function TotemClube({ onCtaClick }: TotemClubeProps) {
  const { totem } = clubeContent;

  return (
    <section className="section-dark py-0 lg:section-padding flex justify-center items-center overflow-x-hidden">
          ...
          <div className="flex-1 flex flex-col justify-center items-center lg:items-start w-full lg:w-1/2 space-y-4 md:space-y-5 text-center lg:text-left order-first lg:order-first">
            ...
            <div className="space-y-4 mt-4 max-w-lg lg:max-w-none">
              {totem.features.map((feature, index) => (
                <div
                  key={index}
                  className="flex items-start space-x-3 md:space-x-4 text-left"
                >
